...
public:
		...
		:counter_clockwise(_counter_clockwise),use_accel_structure(_use_accel_structure)
	{
		...
		if(calculate_smoothed_normal){
			mesh.Build();	////compute the normal in STTriangleMesh
			if(!counter_clockwise){
				for(int i=0;i<(int)mesh.mFaces.size();i++){
					for(int d=0;d<3;d++){
						mesh.mFaces[i]->v[d]->normal*=-1.;
					}
				}
			}
		}
		for(int i=0;i<(int)mesh.mFaces.size();i++){
			...
			if(read_normal_from_obj||calculate_smoothed_normal){
				for(int d=0;d<3;d++){n[d]=mesh.mFaces[i]->v[d]->normal;}
			}
			else{
				STVector3 n_tri = STVector3::Cross(v[1] - v[0], v[2] - v[1]); //going counterclockwise
				n_tri.Normalize();
				for(int d=0;d<3;d++){n[d]=n_tri;}
			}
