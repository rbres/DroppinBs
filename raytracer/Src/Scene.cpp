...
STColor3f Scene::TraceRay(const Ray &ray, int bounce) {
    ...
    STColor3f result = !use_transparent_shadow ? object->material.shade(inter, ray.d * -1., visibleLights, object->material.isMatte() ? STColor3f() : TraceRay(*reflected, bounce + 1), refracted ? TraceRay(*refracted, bounce + 1) : STColor3f()) :
		object->material.shade(inter, ray.d * -1., visibleLights, attenuation, object->material.isMatte() ? STColor3f() : TraceRay(*reflected, bounce + 1), refracted ? TraceRay(*refracted, bounce + 1) : STColor3f());
...
{
	if(currMaterial == NULL)currMaterial=new Material(mat.ambient,mat.diffuse,mat.specular,mat.mirror,mat.shininess,mat.refract,mat.snell);
	else{
...
void Scene::rtLoadTexture(const std::string image_name)
{
	textures.push_back(new STImage(image_name));
}
